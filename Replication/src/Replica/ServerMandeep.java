...
public class ServerMandeep implements Runnable{
	...
	public static ServerMandeep Replica;
	public static ServerMandeep MTLServer;
	public static ServerMandeep DDOServer;
	public static ServerMandeep LVLServer;
	...
	//initializing
	public ServerMandeep(String L, int P){
	...
	public void transferRecord(String managerID, String recordID, String remoteClinicServerName) 
	{
		...
		if(remoteClinicServerName.equals("MTL")){
			remoteport = Config.PORT_NUMBER_MTL_Mandeep;
		}
		else if(remoteClinicServerName.equals("DDO")){
			remoteport = Config.PORT_NUMBER_DDO_Mandeep;
		}
		else{
			remoteport = Config.PORT_NUMBER_LVL_Mandeep;
		}
	 ...
	 */
	public String action(ServerMandeep server, String s){
		String reply = null;
		String[] array = s.split(",,");
				switch(array[2]){
					case "1":{
						reply = "FE,,ReplicaMandeep,,1,,The Connection was Confirmed by the server in "+ server.currentLocation+",,";
						break;
					}
					case "2":{

						loggerServer.info("ReplicaMandeep Says: "+"Create Doctor Operation is Invoked at server of " + server.currentLocation + " by manager with manager-id" + array[9]);
						
						server.createDoctorRecord(array[3], array[4], array[5], array[6], array[7], server.currentLocation, array[9]);
						
						loggerSystem.info("ReplicaMandeep Says: "+"New Doctor Record is created by manager with managerID "
								 + array[9] + " in clinic location " + server.currentLocation);
						
						reply = "FE,,ReplicaMandeep,,2,,added records are: " + server.getRecord()+" and the currentcount is: " + server.currentCount+",,";
						break;
					}
					case "3":{
						
						loggerServer.info("ReplicaMandeep Says: "+"Create Nurse Operation is Invoked at server of " + server.currentLocation + " by manager with manager-id" + array[9]);
						
						server.createNurseRecord(array[3], array[4], array[5], array[6], array[7], server.currentLocation, array[9]);
						
						loggerSystem.info("ReplicaMandeep Says: "+"New Nurse Record is created by manager with managerID "
								 + array[9] + " in clinic location " + server.currentLocation);
						
						reply = "FE,,ReplicaMandeep,,3,,added records are: " + server.getRecord()+" and the currentcount is: " + server.currentCount+",,";
						break;
					}
					case "4":{
						
						loggerServer.info("ReplicaMandeep Says: "+" Record Info Operation is Invoked at server of " + server.currentLocation);
						
						reply = "FE,,ReplicaMandeep,,4,," + server.getRecord()+",,";
						break;
					}
					case "5":{
						loggerServer.info("ReplicaMandeep Says: "+"Edit Record Operation is Invoked at server of " + server.currentLocation);
						...
						if (array[3].substring(0, 1).equals("D")){
							...
							reply = "FE,,ReplicaMandeep,,5" + ",, Doctor Record with the ID of " +array[3]+ " was edited and the value of field "+ array[4]+ 
							...
							loggerSystem.info("ReplicaMandeep Says: "+",, Doctor Record with the ID of " +array[3]+ " was edited and the value of field "+ array[4]+ 
									" was successfully changed to " + array[5]+ ". The current record details are: First Name: " + doctor.FirstName + 
									" Last Name: " + doctor.LastName + " Address: " + doctor.Address + " Phone Number: " + doctor.PhoneNumber + " Specialization: " + 
									doctor.Specialization + ",,");
						}
						...
						if (array[3].substring(0, 1).equals("N")){
							...
							reply = "FE,,ReplicaMandeep,,5" + ",, Nurse Record with the ID of " +array[3]+ " was edited and the value of field "+ array[4]+ 
							...
							loggerSystem.info("ReplicaMandeep Says: "+",, Nurse Record with the ID of " +array[3]+ " was edited and the value of field "+ array[4]+ 
									" was successfully changed to "	+ array[5]+ ". The current record details are: First Name: " + nurse.FirstName + 
									" Last Name: " + nurse.LastName + " Designation: " + nurse.designation	+ " Status: " + nurse.status + " Status Date: " + 
									nurse.statusDate + ",,");
						}
					...
					case "6":{
						...
						reply = "FE,,ReplicaMandeep,,6,,The record with ID: "+array[3]+" is being transfered from "+server.currentLocation+" to " + array[4]+",,";
						
						loggerSystem.info("ReplicaMandeep Says: "+"The record with ID: "+array[3]+" is being transfered from "+server.currentLocation+" to " + array[4]+",,");
						break;
					}
					case "7":{
						loggerServer.info("ReplicaMandeep Says: "+"Total Record Operation is Invoked at server of " + server.currentLocation);
						
						reply = "FE,,ReplicaMandeep,,7,," + MTLcount+",," + 
								DDOcount+",," + LVLcount+",,";
						
						loggerSystem.info("ReplicaMandeep Says: "+"MTLCount: "+MTLcount+" DDOCount: " + 
					...
					case "D":{
						...
						reply = "FE,,ReplicaMandeep,,8,,added records are: " + server.getRecord()+" and the currentcount is: " + server.currentCount+",,";

						loggerSystem.info("ReplicaMandeep Says: "+server.currentLocation+":"+"added records are: " + server.getRecord()+" and the currentcount is: " + server.currentCount+",,");
						break;
					}
					case "N":{
						loggerServer.info("ReplicaMandeep Says: "+"Create Nurse Operation is Invoked at server of " + server.currentLocation + " by manager with manager-id" + array[9]);
		...
		int myport=0;
		 ...
		 String[] receivedarray = receivedmsg.split(",,");
				...
				if(receivedarray[1].equals("MTL")){
					myport=Config.PORT_NUMBER_MTL_Mandeep;
				}
				else if(receivedarray[1].equals("DDO")){
					
					myport=Config.PORT_NUMBER_DDO_Mandeep;
				}
				else if(receivedarray[1].equals("LVL")){
					myport=Config.PORT_NUMBER_LVL_Mandeep;
				}
				else{
	...
	public void run() {
		...
		try
		{
			...
			while(true)
			{
				...
				if(this.udpserverport == Config.PORT_NUMBER_ReplicaMandeep){
					...
					if(myrequestMULTI.getPort()==Config.PORT_NUMBER_FrontEnd){
						...
						//if the message is from RM (the case of crash confirmation)
							...
							myport=Config.PORT_NUMBER_RMMandeep;
						}
				...
				//if it is a server in either MTL, DDO or LVL.
					...
					DatagramPacket myreply = new DatagramPacket(servereply.getBytes(),
							servereply.length(),InetAddress.getByName(Config.IP),Config.PORT_NUMBER_ReplicaMandeep);
					mySocket.send(myreply);
					receivedmsg = null;
				}
				}
